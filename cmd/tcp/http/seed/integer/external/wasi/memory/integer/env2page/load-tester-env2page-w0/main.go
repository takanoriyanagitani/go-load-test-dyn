...
import (
	...
	w1 "github.com/tetratelabs/wazero/imports/wasi_snapshot_preview1"
...
var envKeys string = getenvOrAlt("ENV_KEYS_TO_WASI", "")
...
func reduce[T, S any](s []T, reducer func(S, T) S, init S) S {
	var state = init
	for _, i := range s {
...
func configWithEnvVars(
	envKeys []string,
	base wz.ModuleConfig,
	alt string,
) wz.ModuleConfig {
	return reduce(
		envKeys,
...
func (cfg ConfigCreator) ToConfig() wz.ModuleConfig {
...
var cfg wz.ModuleConfig = ConfigCreator{
	EnvKeys: strings.Split(envKeys, ","),
	Base:    wz.NewModuleConfig().WithStartFunctions(),
	EnvAlt:  "",
}.ToConfig()
